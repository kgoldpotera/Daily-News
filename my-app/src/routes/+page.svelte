<script lang="ts">
  ...
  let timeFilter: TimeOpt = '24h'; // start wide so we see items
  

  function withinRange(iso: string) {
    const hours: number = timeFilter === '1h' ? 1 : timeFilter === '3h' ? 3 : 24;
    const t = new Date(iso).getTime();
    if (Number.isNaN(t)) return true; // if date is odd, don't drop it
  ...
  async function refresh() {
    ...
    // Fetch Kenya
    try {
      const keR = await fetch('/api/kenya');
      const keJ = await keR.json();
    ...
    // Fetch Global
    try {
      const glR = await fetch('/api/global');
      const glJ = await glR.json();
    ...
    errorMsg = errs.join(' | ');
    // console.log('refresh done', { kenya: kenya.length, global: globalBrief.length, errorMsg });
  }
...
<main class="container">
  ...
  <section style="display:flex; gap:8px; flex-wrap:wrap; margin:12px 0;">
